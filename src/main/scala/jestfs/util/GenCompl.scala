...
object GenCompl {
  ...
  // generate completion file "$JESTFS_HOME/.completion"
  def main(args: Array[String]): Unit = dumpFile(content, path)

  // commands
  val commands = JestFs.commands

  // phases
  val phases = JestFs.phases

  // global options
  val options = JestFs.options
  ...
  def content: String = {
    ...
    |_jestfs_completions() """.stripMargin
    app.wrap {
    ...
    app :> "complete -o filenames -o bashdefault -F _jestfs_completions jestfs"
