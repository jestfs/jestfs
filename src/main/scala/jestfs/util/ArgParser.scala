...
import jestfs.{JestFs, Command, CommandConfig}
import jestfs.phase.PhaseOption
import jestfs.error.*
...
class ArgParser(cmd: Command[_], cmdConfig: CommandConfig)
  ...
  addRule(cmdConfig, "", JestFs.options)
              ...
              case obj if json.isObject =>
                obj.asObject.get.toList.foreach {
                  case (phase, value) if value.isObject => {
                    if (JestFs.phases.map(_.name).contains(phase))
