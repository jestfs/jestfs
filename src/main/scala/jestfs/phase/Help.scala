...
import jestfs.{LINE_SEP, Command, VERSION}
import jestfs.util.*
import jestfs.{JestFs, CommandConfig}
...
case object Help extends Phase[Unit, Unit]:
  ...
  def cmdHelp(cmd: Command[_]): String =
    ...
    app :> ""
    app :> "Usage: jestfs " >> cmd.name >> " <option>*"
  ...
  lazy val helpMessage =
    val app = Appender()
    app >> "`jestfs` - JavaScript conformance test generator using feature-sensitive coverage"
    app :> ""
    app :> "Usage: jestfs <command> <option>* <filename>*"
    app :> ""
    app :> "If you want to see the detailed usage of each command,"
    app :> "please type `jestfs help <command>`."
    app :> ""
    app :> "- command list:"
    app :> "    Each command consists of following phases."
    app :> "    format: <command> <phase> [>> {phase}]*"
    app :> ""
    for (cmd <- JestFs.commands)
      header(app, cmd.name)
      body(app, cmd.help)
      body(app, s"(${cmd.pList})", true)
      app :> ""

    addPhase(app, JestFs.phases)
    addGlobalOption(app)
    app.toString
  ...
  def addGlobalOption(app: Appender): Unit =
    app :> "- global option:"
    for ((opt, kind, desc) <- JestFs.options)
